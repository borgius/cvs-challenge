...
For the first version of PR Concierge, I chose API Gateway plus Lambda so the service could stay small, event-driven, and easy to deploy. The workload only wakes up for GitHub pull request webhooks, does a short burst of deterministic work, and then exits. That matches Lambda well and keeps the operational surface small while the core product value is still being proven. It also keeps the repository reviewable: one lightweight TypeScript runtime, a simple HTTP entrypoint, and an AWS footprint that is easy to explain during the technical interview.
